...
@section('title', 'إدارة تقييمات الموردين')
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">تقييمات الموردين</h3>
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div>
                                            <div class="fw-bold">{{ $review->vendor->name }}</div>
                                            <small class="text-muted">{{ $review->vendor->business_name }}</small>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="d-flex align-items-center">
                                    ...
                                    <td>
                                        <div class="btn-group" role="group">
                                            @if(!$review->is_approved)
                                                <form method="POST" action="{{ route('admin.reviews.vendors.approve', $review->id) }}" class="d-inline">
                                                    @csrf
                                            ...
                                            @if($review->is_approved)
                                                <form method="POST" action="{{ route('admin.reviews.vendors.reject', $review->id) }}" class="d-inline">
                                                    @csrf
                                            ...
                                            <form method="POST" action="{{ route('admin.reviews.vendors.delete', $review->id) }}" class="d-inline" onsubmit="return confirm('هل أنت متأكد من حذف هذا التقييم؟')">
                                                @csrf
                                                @method('DELETE')
