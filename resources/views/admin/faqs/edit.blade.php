        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">تعديل السؤال الشائع</h3>
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('question_ar') is-invalid @enderror" 
                                           id="question_ar" name="question_ar" value="{{ old('question_ar', $faq->question_ar) }}" required>
                                    @error('question_ar')
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('category') is-invalid @enderror" 
                                        ...
                                        <option value="orders" {{ old('category', $faq->category) == 'orders' ? 'selected' : '' }}>الطلبات</option>
                                        <option value="payment" {{ old('category', $faq->category) == 'payment' ? 'selected' : '' }}>الدفع</option>
                                        <option value="delivery" {{ old('category', $faq->category) == 'delivery' ? 'selected' : '' }}>التوصيل</option>
                                        <option value="products" {{ old('category', $faq->category) == 'products' ? 'selected' : '' }}>المنتجات</option>
                                        <option value="support" {{ old('category', $faq->category) == 'support' ? 'selected' : '' }}>الدعم</option>
                                    </select>
                                    @error('category')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
